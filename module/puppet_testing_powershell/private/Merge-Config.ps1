function Merge-Config {
  ...
  foreach ($var in $child.env) {
    if ($baseVar = $base.env | Where-Object -Property 'name' -EQ -Value $var.name) {
      Write-Verbose ("Merge-Config: Found variable: {0} with value: {1} in repo scoped file." -f $var.name, $var.value)
    ...
    if ($baseOverrideSet = $base.env_override_sets | Where-Object -Property 'name' -EQ -Value $overrideSet.name) {
      foreach ($overrideItem in $overrideSet.values) {
        if ($baseOverrideItem = $baseOverrideSet.values | Where-Object -Property 'name' -EQ -Value $overrideItem.name) {
          Write-Verbose ("Merge-Config: Found environment variable override item in set {0} with name {1} with value {2} in repo scoped file" -f $overrideSet.name, $overrideItem.name, $overrideItem.value.ToString())
          $baseOverrideItem.value = $overrideItem.value
        }
        else {
          Write-Verbose ("Merge-Config: Adding override item from set {0} with name {1} and value {2} from repo scoped file." -f $overrideSet.name, $overrideItem.name, $overrideItem.value)
          $baseOverrideSet.values += $overrideItem
        }
      }
    }
    else {
      Write-Verbose ("Merge-Config: Adding override set {0} from repo scoped file." -f $overrideSet.name)
    ...
    if ($baseHypervisor = $baseHypervisors | Where-Object -Property 'name' -EQ -Value $hypervisor.name) {
      foreach ($string in $hypervisor.strings) {
        if ($baseString = $baseHypervisor.strings | Where-Object -Property 'name' -EQ -Value $string.name) {
          Write-Verbose ("Merge-Config: Found Hypervisor string {0} with value {1} for hypervisor {2} in repo scoped file." -f $string.name, $string.value, $hypervisor.name)
          $baseString.value = $string.value
        }
        else {
          Write-Verbose ("Merge-Config: Adding Hypervisor string {0} with value {1} for hypervisor {2} from repo scoped file" -f $string.name, $string.value, $hypervisor.name)
          $baseHypervisor.strings += $string
        }
      }
    }
    else {
      Write-Verbose ("Merge-Config: Adding Hypervisor string set {0} from repo scoped file." -f $hypervisor.name)
